...
func (this *BlockChain) FindSuitableUTXOsFromUTXOSet(address string, amount float64) (validUTXOs map[string][]int64, total float64) {
	...
	for _, superOutput := range tXOutPuts.UTXOs {
		if total < amount {
			...
			validUTXOs[string(superOutput.TXID)] = append(validUTXOs[string(superOutput.TXID)], int64(superOutput.Vout))
