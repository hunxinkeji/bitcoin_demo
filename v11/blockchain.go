...
func InitBlockChain(address string, data string, nodeID string) *BlockChain {
	if isDBExist(nodeID) {
...
func GetBlockChainHandler(nodeID string) *BlockChain {
	if !isDBExist(nodeID) {
...
func (this *BlockChain) FindUTXO(address string) (utxos SuperUTXOs) {
	//存储使用过的utxo的集合 map[交易id][]int64
	//用来记录已经消费掉的output
	spentUTXO := make(map[string][]int64)
	...
	for it.CurrHash != nil {
		...
		OUTPUTS:
			...
			for currIndex, output := range tx.TXOutputs {
				...
				superOutPut := SuperOutPut{
					TXID:   tx.TXID,
					Vout:   int64(currIndex),
					Output: output,
				}
				utxos.UTXOs = append(utxos.UTXOs, superOutPut)
			}
		}
	}
...
func (this *BlockChain) FindSuitableUTXOsFromUTXOSet(address string, amount float64) (validUTXOs map[string][]OutputInfo, total float64) {
	validUTXOs = make(map[string][]OutputInfo)
	fmt.Println("FindSuitableUTXOsFromUTXOSet")

	//通过UTXOSET查找address所对应的SuperOutput
	utxoSet := &UTXOSet{this}
	tXOutPuts := utxoSet.FindUTXOsWithAddressFromUTXOSet(address)
	for _, superOutput := range tXOutPuts.UTXOs {
		if total < amount {
			//validUTXOs map[string][]int64
			outputInfo := OutputInfo{
				Vout:          superOutput.Vout,
				Ripemd160Hash: superOutput.Output.Ripemd160Hash,
			}
			validUTXOs[string(superOutput.TXID)] = append(validUTXOs[string(superOutput.TXID)], outputInfo)
			total += superOutput.Output.Value
		} else {
			return
		}
	}
...
// 找到所有地址所对应的UTXO
func (this *BlockChain) FindUTXOMap() map[string]SuperUTXOs {
	res := make(map[string]SuperUTXOs)
	it := this.NewBlockChainIterator()
	for it.CurrHash != nil {
		block := it.Next()
		for _, tx := range block.Transactions {
			for _, output := range tx.TXOutputs {
				ripemd160Hash := output.Ripemd160Hash
				//根据Ripemd160Hash求出地址
				version_ripemd160Hash := append([]byte{version}, ripemd160Hash...)
				checkSumBytes := CheckSum(version_ripemd160Hash)
				bytes := append(version_ripemd160Hash, checkSumBytes...)
				address := string(Base58Encode(bytes))
				res[address] = this.FindUTXO(address) //(utxos []TXOutput)
			}
		}
	}
	return res
...
func (this *BlockChain) GetBlockHashes() [][]byte {
	var blockHashes [][]byte
	blockIterator := this.NewBlockChainIterator()
	for blockIterator.CurrHash != nil {
...
func (this *BlockChain) GetBlock(ID []byte) (*Block, error) {
	blockIterator := this.NewBlockChainIterator()
	flag := false
	for blockIterator.CurrHash != nil {
		if bytes.Compare(ID, blockIterator.CurrHash) == 0 {
			flag = true
			break
		}
		blockIterator.Next()
	...
	if flag {
		...
		return nil, fmt.Errorf("cannnot find this block")
	}
}
